...
from ..forms import BaseAnswerInlineFormSet, QuestionForm, TeacherSignUpForm, NoticeCreateForm
from ..models import Answer, Question, Quiz, User, c_notice, g_notice
...
class NoticeListView(ListView):
    ...
    template_name = 'classroom/teachers/notices/notice_change_list.html'
...
class NoticeCreateView(CreateView):
    model = c_notice
    fields= ('company_name','company_package','branches','per_10th', 'per_12th', 'year_gap','no_backlog','aggregate','add_notice' )
    template_name = 'classroom/teachers/notices/notice_add_form.html'

    def form_valid(self, form):
        notice = form.save(commit=False)
        notice.owner = self.request.user
        notice.save()
        messages.success(self.request, 'The notice was uploaded with success!')
        return redirect('teachers:notice_change', notice.pk)
...
class NoticeDeleteView(DeleteView):
    ...
    template_name = 'classroom/teachers/notices/notice_delete_confirm.html'
    success_url = reverse_lazy('teachers:notice_change_list')

    def delete(self, request, *args, **kwargs):
        notice = self.get_object()
        messages.success(request, 'The notice %s was deleted with success!' % notice.company_name)
        return super().delete(request, *args, **kwargs)

    def get_queryset(self):
        return self.request.user.notices.all()
